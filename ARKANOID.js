...
function Pelota(constructor_pelota) {
	...
	this.rebotePelota = function(){		//Calcula los rebotes de la pelota
		...
		if (VIDAS == 0){
			...
			canvas.fillText ("Pontuação: " +puntos, (WIDTH/2)-(anchoGameOver/2), HEIGHT*0.65);
...
function puntosVidas(){		//Dibuja las vidas y los puntos en la parte superior del canvas
	...
	canvas.fillStyle = "white";
	canvas.fillText ("Vida Extra: "+premioPelota, WIDTH/100*2, (espacioPuntuaciones/100*65)+(MiBarra.barraAlto/2));

	canvas.font = "bold 22px sans-serif";
	canvas.fillStyle = "white";
	canvas.fillText ("Pontos: "+puntos, WIDTH - 180, espacioPuntuaciones/2+7);
